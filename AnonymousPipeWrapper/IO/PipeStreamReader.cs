namespace AnonymousPipeWrapper.IO
{
    using System;
    using System.IO;
    ...
    public class PipeStreamReader<T> where T : class
    {
        ...
        private int ReadLength()
        {
            ...
            var lenbuf = new byte[LengthSize];
            var bytesRead = this.BaseStream.Read(lenbuf, 0, LengthSize);
            if (bytesRead == 0)
            {
            ...
            if (bytesRead != LengthSize)
                throw new IOException(string.Format("Expected {0} bytes but read {1}", LengthSize, bytesRead));
